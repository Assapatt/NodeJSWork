...
http.createServer(function(req, res) {
    ...
    switch (pathName) {
        ...
        case "/detail":
            ...
            showDetail(res);
            // showChapterData(chapterid, res);
            break;
        case "/getDetail":
            let chapterid = req.url.charAt(req.url.length-1);
            showChapterData(chapterid, res);
            break;
        case "/add":
            addChapter(req, res);
...
function showIndex(res) {
    let indexPath = path.join(__dirname, "./chapterList.html");
...
function showImage(pn, res) {
    let imgPath = path.join(__dirname, pn);
...
function showLogin(res) {
    let indexPath = path.join(__dirname, "./login.html");
    let fileContent = fs.readFileSync(indexPath);
    res.writeHead(200, {"Content-type":"text/html"});
    res.write(fileContent);
}

function showListPage(res) {
    let indexPath = path.join(__dirname, "./list.html");
    let fileContent = fs.readFileSync(indexPath);
    res.writeHead(200, {"Content-type":"text/html"});
    res.write(fileContent);
}
...
function showAddChapter(res) {
    let indexPath = path.join(__dirname, "./addChapter.html");
    let fileContent = fs.readFileSync(indexPath);
    res.writeHead(200, {"Content-type":"text/html"});
    res.write(fileContent);
}
...
function getPwd(req, res){
    ...
    req.on("end", function() {
        ...
        for(let i = 0; i < userList.length; i++) {
            if(userList[i].username === dataObj.username && userList[i].pwd === dataObj.pwd) {
...
function showChapterData(chapterid, res) {
    let dataStr = JSON.stringify(chapterList[chapterid-1]);
    res.writeHead(200, {"Content-Type": "application/json"});
    res.write(dataStr);
    res.end();
}
...
function addChapter(req, res) {
    ...
    req.on("end", function() {
        let dataObj = querystring.parse(dataStr);
        let d = new Date();
        let des = dataObj.content.substring(0, 10);
        let newChapter = {
            ...
            "publishTimer": d.getFullYear()+'-'+(d.getMonth()+1)+'-'+d.getDate(),
            "author": "admin",
            "views": 0
        };
